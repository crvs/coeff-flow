...
chain coeff_flow(simplicial_complex& s_comp,  //
    ...
    while (not queue.empty()) {
        ...
        if (is_boundary) {
            ...
            if (predicted_bdry != p[tau_i]) throw no_bounding_chain();
        } else {
            // there is another coface we now focus on it
            double c_p =
                s_comp.boundary_inclusion_index(tau, sigma_p) *
                (p[tau_i] - s_comp.boundary_inclusion_index(tau, sigma) * c);
     ...
     * cout << "sigmas seen: " << seen_sigmas << "/" <<
     * s_comp.get_level_size(s_comp.dimension());
     * cout << " taus seen: " << seen_taus << "/" <<
     * s_comp.get_level_size(s_comp.dimension() - 1);
     * cout << '\n';
     */
