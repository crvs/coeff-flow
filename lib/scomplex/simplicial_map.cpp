...
simplicial_map::simplicial_map(
    ...
    : domain{domain_}, codomain{codomain_}, fwd_map{point_map_} {
    this->generate_bwd_map();
};  // point map

void simplicial_map::generate_bwd_map() {
    size_t num_pts_cod = codomain->get_level_size(0);
    bwd_map = std::vector< size_t >(num_pts_cod, num_pts_cod + 1);
    for (auto pti : fwd_map) {
        bwd_map.at(pti) = pti;
    }
};
...
cell_t simplicial_map::fwd_map_cell(cell_t dom_cell) {
    ...
    for (auto pti : dom_cell) {
        cod_cell.push_back(fwd_map.at(pti));
    }
    return cod_cell;
};
...
cell_t simplicial_map::bwd_map_cell(cell_t cod_cell) {
    cell_t dom_cell;
    size_t lim = codomain->get_level_size(0);
    for (auto pti : dom_cell) {
        if (pti < lim) dom_cell.push_back(bwd_map.at(pti));
    }
    return dom_cell;
};
...
size_t simplicial_map::fwd_map_cell_index(size_t level, size_t celli) {
    ...
    cell_t cod_cell = fwd_map_cell(dom_cell);
    return codomain->cell_to_index(cod_cell);
};

size_t simplicial_map::bwd_map_cell_index(size_t level, size_t celli) {
    cell_t cod_cell = codomain->index_to_cell(level, celli);
    cell_t dom_cell = bwd_map_cell(cod_cell);
    return domain->cell_to_index(dom_cell);
};
